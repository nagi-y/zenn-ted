...
**重要な設計判断:** チームメイトを起動するときに渡す感情状態は、テッドの現在の感情ではなく、**タスクの文脈（刺激）に対する各チームメイトの初期感情**を推定して渡す。テッドがexcitement全開でも、ヒビキにはそのタスクがどう映るかを `--stimulus` で指定する。テッドの感情をそのまま渡すと、全員がテッドの温度で起動してしまう。
